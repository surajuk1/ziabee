...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$sql .= "`stock_status_id`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`date_available`,`weight`,`weight_class_id`,`status`,";
		$sql .= "`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`) VALUES ";
		$sql .= "($product_id,$quantity,";
		// $sql .= in_array('ean',$product_fields) ? "'$ean'," : "";
		// $sql .= in_array('jan',$product_fields) ? "'$jan'," : "";
		// $sql .= in_array('isbn',$product_fields) ? "'$isbn'," : "";
		// $sql .= in_array('mpn',$product_fields) ? "'$mpn'," : "";
		$sql .= "$stock_status_id,'$model',$manufacturer_id,'$image',$shipping,$price,";
		//$sql .= ($date_added=='NOW()') ? "$date_added," : "'$date_added',";
		$sql .= ($date_available=='NOW()') ? "$date_available," : "'$date_available',";
		$sql .= "$weight,$weight_class_id,$status,";
		$sql .= "$viewed,$length,$width,$height,'$length_class_id','$sort_order','$subtract','$minimum');";
		$this->db->query($sql);
		foreach ($languages as $language) {
			$language_code = $language['code'];
			$language_id = $language['language_id'];
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$basicinfos = array();
			while ($this->startsWith($first_row[$j-1],"basicinfo(")) {
				$language_code = substr($first_row[$j-1],strlen("basicinfo("),strlen($first_row[$j-1])-strlen("basicinfo(")-1);
				$basicinfo = $this->getCell($data,$i,$j++);
				$basicinfo = htmlspecialchars( $basicinfo );
				$basicinfos[$language_code] = $basicinfo;
			}
	...
	protected function validateProducts( &$reader ) {
		...
		// }
		if ($this->use_table_seo_url) {
			$expected_heading = array_merge( $expected_heading, array( "quantity", "model", "manufacturer", "image_name", "shipping", "price", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status",  "description") );
		} else {
			$expected_heading = array_merge( $expected_heading, array( "quantity", "model", "manufacturer", "image_name", "shipping", "price","date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status",  "seo_keyword", "description") );
		}
		if ($exist_meta_title) {
		...
		if ($exist_meta_title) {
			$expected_multilingual = array( "name", "basicinfo","description", "meta_title", "meta_description", "meta_keywords", "tags" );
		} else {
			$expected_multilingual = array( "name", "basicinfo","description", "meta_description", "meta_keywords", "tags" );
		}
		die(print_r($data));
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
...
//		}
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($product_descriptions[$language_code][$key])) {
					...
					$results->rows[$key]['basicinfo'][$language_code] = $product_descriptions[$language_code][$key]['basicinfo'];
					...
					$results->rows[$key]['name'][$language_code] = '';
					$results->rows[$key]['basicinfo'][$language_code] = '';
					$results->rows[$key]['description'][$language_code] = '';
					if ($exist_meta_title) {
						$results->rows[$key]['meta_title'][$language_code] = '';
					}
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		foreach ($languages as $language) {
			$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('basicinfo')+4,30)+1);
		}
		...
		foreach ($languages as $language) {
			$styles[$j] = &$text_format;
			$data[$j++] = 'name('.$language['code'].')';
			$data[$j++] = 'basicinfo('.$language['code'].')';
		}
		$styles[$j] = &$text_format;
		$data[$j++] = 'categories';
		$styles[$j] = &$text_format;
		...
		foreach ($products as $row) {
			...
			foreach ($languages as $language) {
				$data[$j++] = html_entity_decode($row['name'][$language['code']],ENT_QUOTES,'UTF-8');
				$data[$j++] = html_entity_decode($row['basicinfo'][$language['code']],ENT_QUOTES,'UTF-8');
			}
	...
	public function getNotifications() {
		...
		$result = 'The new product will be different from available IDs';
