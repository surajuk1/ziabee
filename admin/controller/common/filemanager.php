...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (isset($this->request->get['filter_name'])) {
			$filter_name = $this->my_Sql_regcase(rtrim(str_replace(array('*', '/', '\\'), '', $this->request->get['filter_name']), '/'));
		} else {
			$filter_name = null;
		}
		...
		// if (substr(str_replace('\\', '/', realpath($directory . '/' . $filter_name)), 0, strlen(DIR_IMAGE . 'catalog')) == DIR_IMAGE . 'catalog') {
			// Get directories
			$directories = $this->rglob($directory . '/*' . $filter_name . '*', GLOB_ONLYDIR);
			...
			// Get files
			$files = $this->rglob($directory . '/*' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF}', GLOB_BRACE);
