...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
		...
		//Banner Image
		if (isset($this->request->post['hiddenbanner'])) {
			$data['hiddenbanner'] = $this->request->post['hiddenbanner'];
		} elseif (!empty($category_info)) {
			$data['hiddenbanner'] = $category_info['banner_image'];
		} else {
			$data['hiddenbanner'] = '';
		}
		...
		if (isset($this->request->post['hiddenbanner']) && is_file(DIR_IMAGE . $this->request->post['hiddenbanner'])) {
			$data['thumbbanner'] = $this->model_tool_image->resize($this->request->post['hiddenbanner'], 100, 100);
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['banner_image'])) {
			$data['thumbbanner'] = $this->model_tool_image->resize($category_info['banner_image'], 100, 100);
		} else {
			$data['thumbbanner'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		if (isset($this->request->post['isbanner'])) {
			$data['isbanner'] = $this->request->post['isbanner'];
		} elseif (!empty($category_info)) {
			$data['isbanner'] = $category_info['is_banner'];
		} else {
			$data['isbanner'] = 0;
		}
