...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$sofaCat = $this->model_catalog_product->getSofaCategory($product_id);
		$data['sofaCat'] = $sofaCat;
		if ($product_info) {
			$url = '';
			...
			$data['area'] = $product_info['area'];
			$data['width'] = $product_info['width'];
			$data['height'] = $product_info['height'];
			$data['depth'] = $product_info['depth'];
			$data['length_value'] = $product_info['length_value'];
			$data['length_class'] = $this->length->getUnit($product_info['length_class_id']);
			
			$data['description'] = html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8');
			$data['basicinfo'] = html_entity_decode($product_info['basicinfo'], ENT_QUOTES, 'UTF-8');
			...
			if ($product_info['image']) {
				$data['popup'] = $this->model_tool_image->resize($product_info['image'], 800, 533);
			} else {
				$data['popup'] = $this->model_tool_image->resize('placeholder.png', 800, 533);
			}

			if ($product_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'], 800, 533);
			} else {
				$data['thumb'] = $this->model_tool_image->resize('placeholder.png', 800, 533);
			}
...
*/
			foreach ($results as $result) {
				if($result['image']!='') {
					$data['images'][] = array(
						'popup' => $this->model_tool_image->resize($result['image'], 800, 533),
						'thumb' => $this->model_tool_image->resize($result['image'], 800, 533)
					);
				} else {
					$data['images'][] = array(
						'popup' => $this->model_tool_image->resize('placeholder.png', 800, 533),
						'thumb' => $this->model_tool_image->resize('placeholder.png', 800, 533)
					);	
				}
			}
