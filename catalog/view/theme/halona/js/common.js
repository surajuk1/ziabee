...
$(document).ready(function() {
	...
	//screen fit
	if ((screen.width<=768)) {
		$('.search-box').removeClass('pull-right');
	} else {
		$('.search-box').addClass('pull-right');
	}
				...
				if (json['success']) {
					...
					$.notify({
				     	...
				     	template: '<div data-notify="container" class="col-xs-11 col-sm-3 alert alert-danger" role="alert">' +
				     		'<button type="button" aria-hidden="true" class="close" data-notify="dismiss">×</button>' +
				     		'<span data-notify="message"><i class="fa fa fa-close"></i>&nbsp; {2}</span>' +
				     		'<div class="progress" data-notify="progressbar">' +
	...
	'add': function(product_id) {
		$.ajax({
			...
			success: function(json) {							
				...
				$('#total_count_wish_ajax').html(json['totalcount']);
			}
		});
	},
	'remove': function() {
...
$(document).ready(function() {
	...
	$('#cart_block #total_price_ajax').load('index.php?route=common/cart/info #total_price');
	$('#cart_block .cart-count').load('index.php?route=common/cart/info #total_count_ajax');
	if(parseInt($('#total_count_ajax').text())>0) {
		$('#cart_block .cart-count').show();			
	} else {
		$('#cart_block .cart-count').show();
	}
