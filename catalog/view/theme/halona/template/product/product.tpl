...
<?php 
echo '<script type="text/javascript" src="catalog/view/theme/' . $config->get('theme_' . $config->get('config_theme') . '_directory') . '/js/owl.carousel2.js"></script>';
echo '<link rel="stylesheet" type="text/css" href="catalog/view/theme/' . $config->get('theme_' . $config->get('config_theme') . '_directory') . '/css/owl.carousel.css" />';
...
<div class="margin-class">
  ...
  <div class="product-info">
  	<div class="row">
<!--   	     <center><a target="_blank" href="http://gccworld.info/doc/halona/"><b><u>Theme Installation Error: Delete Cache or Re-Download and re-upload or check documentation here</u></b></a></center> -->
  		...
  		<div class="col-md-<?php if($theme_options->get( 'custom_block', 'product_page', $config->get( 'config_language_id' ), 'status' ) == 1 || count($product_custom_block)) { echo 9; } else { echo 12; } ?> col-sm-12">
  			<div class="row" id="quickview_product">
			    ...
			    <div class="col-sm-<?php echo $image_grid; ?> popup-gallery">
			      ...
			      <div class="row margin-right20">


				      <?php if (($images || $theme_options->get( 'product_image_zoom' ) != 2) && $theme_options->get( 'position_image_additional' ) != 2 && COUNT($images) >= 1 || true) { ?>
				      <div class="col-sm-12">
                ...
                <div class="content-product-left">
                    ...
                    <?php if ($images) : ?>
                        <div id="thumb-slider" class="thumb-vertical-outer">
								...
								<?php }else { ?>
								<li class="owl2-item">
									<a data-index="<?php echo $i; ?>" class="img thumbnail" data-image="<?php echo $popup; ?>" title="<?php echo $heading_title; ?>">
			...
			$(this).find("[data-index='0']").addClass('active');
        			...
        			<div align="center">
                        <span style="color: #555!important;">Roll over image to zoom in</span>
                    </div>
			    ...
			    <div class="col-sm-<?php echo $product_center_grid; ?> product-center clearfix">
			     <div itemscope itemtype="http://schema.org/Offer">
			      ...
			      <div class="description">
			        ...
			        <?php if($sofaCat): ?>
						<br /><span><?php echo 'Width' ?></span> <?php echo round($width,2)." ".$length_class; ?>
						<br /><span><?php echo 'Height' ?></span> <?php echo round($height,2)." ".$length_class; ?>
						<br /><span><?php echo 'Depth' ?></span> <?php echo round($depth,2)." ".$length_class; ?>
					<?php endif; ?>
			    	</div>
			      <?php if ($price) { ?>
			      <div class="price">
			      ...
			      <div class="cart">
			        <div class="add-to-cart clearfix">
			          ...
			          if( count($product_enquiry) ) { 
     			         ...
     			         <div id="button-add-cart" class="add-tocart"><i class="fa fa-shopping-cart"></i> <input type="button" value="<?php echo $button_cart; ?>" id="button-cart" rel="<?php echo $product_id; ?>" data-loading-text="<?php echo $text_loading; ?>" class="button" /></div>
     			          <div class="cart-minimum"></div>
     			          <?php 
					...
					<?php if(!$sofaCat):?>
			        <div class="feet-link">
 						<input type="text" name="area" id="area_wanted" value="<?php echo round($area,2); ?>" /> <label><h4> Area in sq.ft</h4></label>
			        </div>
			        <?php endif;?>	        
  ...
  <div class="htabs">
  	...
  	<div id="tab-description" class="tab-basicinfo" itemprop="description"><?php echo $basicinfo; ?></div>
  </div>
  <?php endif; ?>
...
$('#button-add-cart').on('click', function() {
	var minimum_req = 1;
	quantity_used = parseInt($('#quantity_wanted').val());
	$('.cart-minimum').html();
	<?php if($minimum):?>	
		minimum_req = <?php echo $minimum;?>;
	<?php endif;?>

	if(quantity_used < minimum_req) {
		$('.cart-minimum').html('<span class="text-danger">Minimum Required : '+minimum_req+'</span>');
		return;
	}
...
<script type="text/javascript"><!--
	$(document).ready(function() {
	 ...
	 //        gallery:'thumb-slider',
		...
		// 	borderSize:"1",
		// 	borderColour:"rgba(0,0,0,0.1)",
		// 	boxShadow:"rgba(0, 0, 0, 0.2) 0px 1px 16px",
		// 	zoomWindowWidth     : 500
	 //    });
...
$(document).ready(function() {     
	setInterval(function(){
		$(".cart-minimum > span").fadeOut(500).fadeIn(500)
	}, 1000);
});
//--></script> 
...
$("#area_wanted").keydown(function() {
	setTimeout(function () {
		if($("#area_wanted").val()!='') {
		 <?php if($area):?>
		 	var area_qty = $("#area_wanted").val()/<?php echo $area; ?>;
		 	var quantity = Math.ceil(area_qty);
		 	if(quantity)
...
$("#quantity_wanted").keydown(function() {
	setTimeout(function () {
		if($("#quantity_wanted").val()!='') {
		 <?php if($area):?>
		 	var area_ph = <?php echo $area; ?>;
		 	var area = area_ph * $("#quantity_wanted").val();

		 	var area = Math.ceil(area);
		 	if(area)
		 		$("#area_wanted").val(area);
		 	else
		 		$("#area_wanted").val('');
		 <?php endif;?>
		}
